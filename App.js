...
//const test =JSON.stringify(Surveys.schema);
//alert(test);
...
fetchData(){
    //... fetching logic
	let realms = new Realm({schema: [Surveys],schemaVersion: 3});
 ...
 _onRefresh() { 
  ...
  render() {
	...
	while(x < surveytag_.length){
		...
		fetch('http://192.168.1.38:8080/payroll/insertinfo.php', {	
			  method: 'POST',
			  headers: {
	...
	fetch('http://192.168.1.38:8080/payroll/dbconfig.php',{
        method: 'GET',
        headers: {
        "Accept": "application/json",
        'Content-Type': 'application/json'
